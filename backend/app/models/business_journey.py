...
# Preparation happens before an Agent Turn exists.  Keeping a stable,
# non-null scope for that one fixed index=1 call lets the database retain a
# single cross-dialect uniqueness rule without making NULL uniqueness
# semantics part of the ledger contract.
PREPARATION_SLOT_SCOPE = "__preparation__"
...
class BusinessJourneyModelCall(Base):
    ...
    run_id: Mapped[str] = mapped_column(String(200), nullable=False)
    journey_id: Mapped[str] = mapped_column(String(100), nullable=False)
    # Preparation uses PREPARATION_SLOT_SCOPE; runtime rows use the actual
    # Agent Turn id.  This makes logical indices 2/3 reusable across turns
    # while preserving duplicate protection inside one turn.
