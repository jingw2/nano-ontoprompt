...
class EventIngestService:
    ...
    def _dead_letter_event(self, db: Session, *, run_id: str, reason: str, now: datetime) -> RefreshRun:
        ...
        # that function's comment) — without it, a RefreshRun already in
        # this session's identity map is returned as-is, ignoring the row
        # FOR UPDATE just locked and re-read. Concretely: an operator's
        # cancellation committed by another session between that load and
        # this re-fetch would be silently discarded.
        run = db.execute(
        ...
        if run.status == "cancel_requested":
            ...
            # cancelled transition `process()`'s own cancellation branch uses
            # (contract.py's finalize_refresh_cancellation) instead of
            # overwriting it with dead_lettered and creating a
            # RefreshDeadLetter row an operator could replay, which would
            # make a cancelled run resurrectable.
            try:
